...
class Quotes(commands.Cog):
    ...
    async def addquote(self, ctx: bot.AoiContext, trigger: str, *, response: str):
        rowid = (
            await self.bot.db.conn.execute_insert(
                "insert into quotes (user, guild, name, content) values (?,?,?,?)",
                (ctx.author.id, ctx.guild.id, trigger, response),
            )
        )[0]
        await self.bot.db.conn.commit()
        await ctx.send_ok(
            f"Added quote **#{rowid}** - **{discord.utils.escape_markdown(trigger)}** - "
    ...
    async def listuserquotes(self, ctx: bot.AoiContext, member: discord.Member = None):
        member = member or ctx.author
        await ctx.paginate(
            (
                f"**#{row[0]}** - **{discord.utils.escape_markdown(row[1])}**"
                for row in await self.bot.db.conn.execute_fetchall(
                    "select id, name from quotes where user=? and guild=?",
    ...
    async def searchquotes(self, ctx: bot.AoiContext, *, search_term: str):
        rows = await self.bot.db.conn.execute_fetchall(
            "select id, name from quotes where " "guild=? and content like ?",
