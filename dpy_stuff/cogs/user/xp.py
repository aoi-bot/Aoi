...
class XP(commands.Cog):
    ...
    @commands.is_owner()
    @commands.command(brief="Reloads the xp images")
    async def xpr(self, ctx: bot.AoiContext):
        self.fp.close()
        self.fp_g.close()
        self.fp = open("assets/background.png", "rb")
        self.xp_img: PIL.Image.Image = PIL.Image.open(self.fp)
    ...
    async def _xp_template(
        ...
        draw.text((8, 75), text=f"#{r}", font=_font(24), fill=color)
        draw.text((21, 13), text=str(l), font=self.level_font, fill=color)
        name_font = _font(30)
        ...
        # y 15 x 190 64
        draw.text(
            (190 - name_size / 2, 214 - name_height / 2),
            text=member.name,
            font=name_font,
            fill=color,
        )
    ...
    @commands.command(brief="Gets the XP of a user")
    async def xp(self, ctx: bot.AoiContext, member: discord.Member = None):
        member = member or ctx.author
        if not await ctx.using_embeds():
            level, partial, rank, required = await self._xp_values(
        ...
        buf = io.BytesIO()
        (
            await self._xp_template(
                ctx,
                member,
                self._get_rank,
                (130, 36, 252),
                self.xp_img,
                self.bot.db.xp[ctx.guild.id][member.id],
            )
        ).save(fp=buf, format="PNG")
        buf.seek(0)
        await ctx.send(file=(discord.File(buf, "xp.png")))

    @commands.command(brief="Gets the global XP of a user")
    async def gxp(self, ctx: bot.AoiContext, member: discord.Member = None):
        member = member or ctx.author
        if not await ctx.using_embeds():
            level, partial, rank, required = await self._xp_values(
    ...
    @commands.is_owner()
    @commands.command(brief="Set a user's xp")
    async def setxp(self, ctx: bot.AoiContext, xp: int, member: discord.Member = None):
        member = member or ctx.author
        await self.bot.db.ensure_xp_entry(member)
    ...
    @commands.is_owner()
    @commands.command(brief="Add to a user's xp")
    async def addxp(self, ctx: bot.AoiContext, xp: int, member: discord.Member = None):
        member = member or ctx.author
        await self.bot.db.ensure_xp_entry(member)
        self.bot.db.xp[member.guild.id][member.id] += xp
