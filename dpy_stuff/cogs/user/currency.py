...
class Currency(commands.Cog, CurrencyService):
    ...
    async def award_g(self, ctx: bot.AoiContext, member: Optional[discord.Member], amount: int):
        ...
        await ctx.send_info(
            f"Added ${amount} to {member.mention}. Their new total is "
            f"{await self.bot.db.get_global_currency(member)}."
        )

    @commands.command(brief="Checks your wallet", aliases=["$"])
    async def wallet(self, ctx: bot.AoiContext, member: discord.Member = None):
        member = member or ctx.author
    ...
    async def grab(self, ctx: bot.AoiContext):
