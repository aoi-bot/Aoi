...
class Roles(commands.Cog, ColorService):
    ...
    async def rolerename(self, ctx: bot.AoiContext, role: discord.Role, *, name: str):
        self._check_role(ctx, role)
    ...
    @commands.has_permissions(manage_roles=True)
    @commands.command(
        brief="Creates one or more roles - multiword role names must be quoted.",
        aliases=["cr"],
    )
    async def createrole(self, ctx: bot.AoiContext, names: Greedy[rolename()]):
        async def _(name):
        ...
        if len(names) > 3:
            conf = await ctx.confirm(
                "Create roles: " + (" ".join(f"`{n}`" for n in names) + "?"),
                "Creating roles...",
    ...
    @commands.bot_has_permissions(manage_roles=True)
    @commands.has_permissions(manage_roles=True)
    @commands.command(brief="Deletes one or more roles", aliases=["dr"])
    async def deleterole(self, ctx: bot.AoiContext, roles: Greedy[discord.Role]):
        if not roles:
        ...
        if len(roles) > 3:
            conf = await ctx.confirm(
                f"Delete {' '.join(r.name for r in roles)}?",
                "Deleting roles...",
    ...
    async def rolegradient(
        ...
        async def do_op():
            ...
            for idx, clr in enumerate(colors):
                ...
                img_draw.rectangle(
                    [(idx * 240 / num, 0), ((idx + 1) * 240 / num, 48)],
        ...
        await self.bot.create_task(ctx, do_op(), lambda: f"{n}/{num}")
        await ctx.embed(
            title="Roles colored according to gradient",
    ...
    @commands.command(
        ...
        flags={
            "ignorebots": (None, "Ignore bots"),
            "withrole": (
                discord.Role,
                "Only adds a role to people with the supplied role",
            ),
        },
    )
    async def roleall(self, ctx: bot.AoiContext, role: discord.Role):
        flags = ctx.flags
    ...
    @commands.command(brief="Adds a role users get on join", aliases=["aarole"])
    async def addautorole(self, ctx: bot.AoiContext, role: discord.Role):
        if (ctx.guild.id in self.bot.db.auto_roles) and len(
            self.bot.db.auto_roles[ctx.guild.id]
        ) >= self.bot.config.get(
            "admin.max_autorole"
        ):  # noqa
        ...
        await ctx.send_ok(
            f"{role.mention} added to the list of automatically assigned roles on this server. You can "
    ...
    @commands.has_permissions(manage_roles=True, manage_guild=True)
    @commands.max_concurrency(1, per=commands.BucketType.guild)
    @commands.command(brief="Deletes a role users get on join", aliases=["darole"])
    async def delautorole(self, ctx: bot.AoiContext, role: Union[discord.Role, int]):
        was_role = False
        ...
        await ctx.send_ok(
            f"{'<@&' if was_role else ''}{role}{'>' if was_role else ''} deleted from the list of "
    ...
    async def listautoroles(self, ctx: bot.AoiContext):
        ...
        await ctx.send_info(
            ...
            + "\n".join(f"<@&{i}>" for i in self.bot.db.auto_roles[ctx.guild.id])
        )

    @commands.command(
        brief="Lists the server's roles",
        flags={
            "audit": [None, "View more details"],
            "full": [None, "View full details"],
        },
    )
    async def roles(self, ctx: bot.AoiContext):
        def _(s):
        ...
        if "full" in ctx.flags:
            ...
            await ctx.paginate(
                [
                    ...
                    f"{'c' if r.permissions.manage_channels or r.permissions.administrator else '-'}"
    ...
    async def listselfroles(self, ctx: bot.AoiContext):
        ...
        if lost_roles:
            await ctx.trigger_typing()
        for r in lost_roles:
            await self.bot.db.remove_self_role(r)

        if not await self.bot.db.get_self_roles(ctx.guild):
            return await ctx.send_error(f"{ctx.guild} has no self-assignable roles")

        await ctx.send_info(
            "Self-assignable roles on this server\n"
            + "\n".join(f"<@&{i}>" for i in await self.bot.db.get_self_roles(ctx.guild))
        )
    ...
    @commands.command(brief="Adds a self-assignable role to you")
    async def addrole(self, ctx: bot.AoiContext, *, role: discord.Role):
        if not await self.bot.db.get_self_roles(ctx.guild):
            return await ctx.send_error(f"{ctx.guild} has no self-assignable roles")
        self._soft_check_role(ctx, role, "add")
        # remove invalid self-roles
    ...
    @commands.command(brief="Removes a self-assignable role from you")
    async def removerole(self, ctx: bot.AoiContext, *, role: discord.Role):
        if not await self.bot.db.get_self_roles(ctx.guild):
            return await ctx.send_error(f"{ctx.guild} has no self-assignable roles")
    ...
    @commands.has_permissions(manage_roles=True)
    @commands.bot_has_permissions(manage_roles=True)
    @commands.command(brief="Creates color roles")
    async def colorroles(self, ctx: bot.AoiContext):
        colors = {
            "Red": 0xE74C3C,
            "Orange": 0xE67E22,
            "Yellow": 0xF1C40F,
