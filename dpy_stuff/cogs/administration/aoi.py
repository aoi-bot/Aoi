...
class Bot(commands.Cog):
    ...
    async def invite(self, ctx: bot.AoiContext):
        permissions_int = 268659776
        invite_url = (
            f"https://discord.com/api/oauth2/authorize?client_id={self.bot.user.id}&permissions="
            f"{permissions_int}&scope=bot"
        )
        await ctx.send_info(f"Invite me to your server [here]({invite_url})")

    @commands.command(brief="Shows #BOT#'s latency to discord")
    async def ping(self, ctx: bot.AoiContext):
        await ctx.send_info(f":ping_pong: {round(self.bot.latency * 1000)}ms")

    @commands.is_owner()
    @commands.command(brief="Log #BOT# out")
    async def die(self, ctx: bot.AoiContext):
        await ctx.send_ok("Bye :(")
        await self.bot.db.close()
        await self.bot.close()

    @commands.is_owner()
    @commands.command(brief="Flush XP/Currency to database manually")
    async def flush(self, ctx: bot.AoiContext):
        await self.bot.db.cache_flush()
        await ctx.send_ok("Cache flushed to disk")

    @commands.is_owner()
    @commands.command(brief="List servers the bot is part of", aliases=["listservers"])
    async def guildlist(self, ctx: bot.AoiContext):
        await ctx.paginate(
    ...
    async def guildinfo(self, ctx: bot.AoiContext, guild: int):
    ...
    async def setavatar(self, ctx: bot.AoiContext, *, url: str):
        try:
    ...
    async def setname(self, ctx: bot.AoiContext, *, name: str):
    ...
    async def shards(self, ctx: bot.AoiContext):
        # collect shard data
        closed = 0
    ...
    async def restart(self, ctx: bot.AoiContext):
    ...
    async def reloadshard(self, ctx: bot.AoiContext, shard: int):
    ...
    async def placeholders(self, ctx: bot.AoiContext):
        await ctx.send_info(
    ...
    async def sqlselect(self, ctx: bot.AoiContext, *, sql: str):
        sql = sql_trim(sql)
        try:
        ...
        if not value:
            ...
            try:
                ...
                if key.split(".")[0] not in self._config.yaml:
                    ...
                    return await ctx.send_error(
                        f"**{key}** is an invalid key. Do `{ctx.clean_prefix}botconfig "
