...
class Triggers(commands.Cog):
    ...
    async def removetrigger(self, ctx: bot.AoiContext, trigger_type: str, *, arg: str):
        supported_trigger_types = ["addrole", "removerole"]
        if trigger_type not in supported_trigger_types:
            return await ctx.send_error(
                f"{trigger_type} not a supported trigger type. It must be one of: "
    ...
    async def roletriggers(
        ...
        row = await (
            await self.db.execute(
                ...
                (ctx.guild.id, add_or_remove, for_role.id),
            )
        ).fetchone()
        if not row:
            return await ctx.send_error(
    ...
    async def _append_roleadd_trigger(
        ...
        if write:
            await self.db.execute(
                "DELETE FROM roletriggers WHERE guild=? AND type=? AND role=?",
                (guild, "add", role),
            )
            await self.db.execute(
                "INSERT INTO roletriggers VALUES (?,?,?,?,?)",
