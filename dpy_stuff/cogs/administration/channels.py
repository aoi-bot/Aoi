...
class Channels(commands.Cog):
        ...
        for channel, value in self.slowmodes.items():
            ...
            if not ch:
                ...
                await self.bot.db.conn.execute(
                    "delete from slowmode where channel=?", (channel,)
                )
                await self.bot.db.conn.execute(
                    "delete from last_messages where channel=?", (channel,)
                )
    ...
    async def nsfw(self, ctx: bot.AoiContext, channel: discord.TextChannel = None):
        ...
        await ctx.send_ok(
            f"{channel.mention} has been marked as {'' if channel.nsfw else 'not '} NSFW."
        )
    ...
    async def slowmode(self, ctx: bot.AoiContext, time: t_delta()):
        ...
        if not time.days and time.seconds <= 21600:
            if ctx.channel.id in self.slowmodes:
                ...
                await self.bot.db.conn.execute(
                    "delete from slowmode where channel=?", (ctx.channel.id,)
                )
                await self.bot.db.conn.execute(
                    "delete from last_messages where channel=?", (ctx.channel.id,)
                )
                await self.bot.db.conn.commit()
            await ctx.channel.edit(slowmode_delay=time.seconds)
            return await ctx.send_ok(
                f"Slowmode set to {hms_notation(time.seconds)}"
                if time.total_seconds()
                else "Slowmode turned off."
            )
        ...
        await ctx.send_ok(
            f"Slowmode set to {dhms_notation(time)}"
            if time.total_seconds()
            else "Slowmode turned off"
        )
    ...
    async def on_guild_channel_update(
        ...
        if after.slowmode_delay < 21600:
            if after.id in self.slowmodes:
                ...
                await self.bot.db.conn.execute(
                    "delete from slowmode where channel=?", (after.id,)
                )
                await self.bot.db.conn.execute(
                    "delete from last_messages where channel=?", (after.id,)
                )
                await self.bot.db.conn.commit()
