...
class Guilds(commands.Cog):
    ...
    @commands.command(aliases=["guildnm", "servernm"], brief="Renames the server")
    async def renameserver(self, ctx: bot.AoiContext, *, name: str):
        await ctx.confirm_coro(
            f"Rename server to `{name}`?",
    ...
    @commands.has_permissions(manage_guild=True)
    @commands.command(
        aliases=["guildav", "serverav", "servericon"],
        ...
        async with aiohttp.ClientSession() as sess:
            async with sess.get(url) as resp:
                await ctx.confirm_coro(
                    ...
                    ctx.guild.edit(icon=await resp.content.read()),
                )

    @commands.bot_has_permissions(manage_guild=True)
    @commands.has_permissions(manage_guild=True)
    @commands.command(
        aliases=["guildreg", "serverreg"],
    ...
    async def serverregion(self, ctx: bot.AoiContext, *, region: str):
    ...
    @commands.command(brief="List of regions the server can use")
    async def regions(self, ctx: bot.AoiContext):
        await ctx.send_info(
            "Server regions:\n"
            + "\n".join(
                f"◆ {x}"
    ...
    async def delemoji(
        self,
        ctx: bot.AoiContext,
        emojis: commands.Greedy[Union[discord.Emoji, discord.PartialEmoji]],
    ):
        e: discord.Emoji
        if len(emojis) < 1:
        ...
        for e in emojis:
            if isinstance(e, discord.PartialEmoji) or e.guild_id != ctx.guild.id:
                return await ctx.send_error(
                    f"{e} is not from this server. This command can only be used with emojis "
            ...
            async with aiohttp.ClientSession() as sess:
                try:
                    ...
                    await ctx.send_error(
                        f"I got an error trying to get that image."
