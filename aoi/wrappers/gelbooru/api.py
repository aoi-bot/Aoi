...
class GelbooruBrowser:
    def __init__(self, api_key: str, user_id: str, *, banned_tags: List[str] = []):
        self.banned_tags = banned_tags
        self.api_key = api_key
        self.user_id = user_id

    async def get_posts(
        self, tags: List[str], *, limit: int = 100, page: int = 0
    ) -> Tuple[Optional[List[GelbooruPost]], bool, bool]:
        filtered_tags = []
        posts = []
        filtered_tag = False
        for i in tags:
        ...
        async with aiohttp.ClientSession() as sess:
            url = (
                f"https://gelbooru.com/index.php?tags={'+'.join(filtered_tags)}"
