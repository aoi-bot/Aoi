...
def get_role_members(ctx: tanjun.abc.Context, role: hikari.Role) -> list[hikari.Member]:
    return [
        mem for mem in ctx.get_guild().get_members().values() if role.id in mem.role_ids
    ]


def get_role_member_count(ctx: tanjun.abc.Context, role: hikari.Role) -> int:
    return sum(
        1 if role.id in member.role_ids else 0
...
async def roleinfo(ctx: tanjun.abc.Context, role: hikari.Role):
    await ctx.respond(
        ...
        .add_field(
            name="Members", value=str(get_role_member_count(ctx, role)), inline=True
        )
        ...
        .add_field(
            name="Created at", value=str(role.created_at.strftime("%c")), inline=True
        )
    )
...
async def userinfo(ctx: tanjun.abc.Context, member: hikari.Member):
    ...
    hoisted_roles = [
        r for r in member.get_roles() if r.is_hoisted and r.id != ctx.guild_id
    ]
    normal_roles = [
        r for r in member.get_roles() if not r.is_hoisted and r.id != ctx.guild_id
    ]
    await ctx.respond(
        ...
        .add_field(
            name=f"Joined {ctx.get_guild()}", value=str(member.joined_at.strftime("%c"))
        )
        .add_field(name="Joined Discord", value=str(member.created_at.strftime("%c")))
        .add_field(
            name=f"Hoisted Roles: {len(hoisted_roles)}",
            value=" ".join([r.mention for r in hoisted_roles[:-6:-1]])
            if hoisted_roles
            else "None",
        )
        .add_field(
            name=f"Normal Roles: {len(normal_roles)}",
            value=" ".join([r.mention for r in normal_roles[:-6:-1]])
            if normal_roles
            else "None",
        )
        .add_field(
            name="Top Role",
            value=str(member.get_top_role().mention)
